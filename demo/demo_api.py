...
@app.route('/hoi/predict', methods=['post'])
def infer():
    # import ipdb
    # ipdb.set_trace()

    global G_PREDICTOR
    assert type(G_PREDICTOR) == Demo

    image_base64 = request.json.get('image')
    pred = G_PREDICTOR(root="./demo",
                       source=image_base64,
                       is_base64=True,
                       savename="test.jpg",
                       savejson=False,
    ...
    for p in pred['predictions']:
        boxes.append([float(x) for x in p['bbox']])
        categories.append(int(p['category_id']))
...
if __name__ == '__main__':
    main(pretrained="/home/airport/CDN_mod/work_dir/1206_train_oneobj/checkpoint_best.pth",
         device="cuda:4",
         out_dir="./demo/out",
         root="./demo",
         source=None,
