...
class Predictor:
    ...
    def infer_with_base64(self,encoded_strings):
        """
        ...
        """
        decoded_bytes = base64.b64decode(encoded_strings)
        image = Image.open(BytesIO(decoded_bytes))
        # img_array = np.array(image)
        return image

    def predict(self,source,mode="Base64_test"):
        path = "./data/demo_video_fps2/hico/images/test2015/"
        source = osp.join(path, source)
        print(source)
        with open(source, "rb") as image_file:
            # 将图片文件的内容进行Base64编码
        ...
        if mode=="Base64_test":
            img = self.infer_with_base64(encoded_strings=encoded_string)
        else:
