...
def main(args):
    ...
    dataset_train2 = build_dataset(image_set='train2', args=args)
    dataset_val2 = build_dataset(image_set='val2', args=args)
    ...
    if args.distributed:
        ...
        sampler_val2 = DistributedSampler(dataset_val2, shuffle=False)
    else:
    ...
    data_loader_train2 = DataLoader(dataset_train2, batch_sampler=batch_sampler_train2,
                                   collate_fn=utils.collate_fn, num_workers=args.num_workers)
    data_loader_val2 = DataLoader(dataset_val2, args.batch_size, sampler=sampler_val2,
                                 drop_last=False, collate_fn=utils.collate_fn, num_workers=args.num_workers)
    if args.frozen_weights is not None:
    ...
    if args.resume:
        ...
        if False:
            ...
            for name, param in checkpoint['model'].items():
                if name in model_without_ddp.state_dict():
                    try:
                        ...
                        print("===!=======!=========!===")
                        print("An unexcepted layer !!!!!!")
                        print(name)
                        print("==========================")
                        if 'weight' in name:
                            nn.init.normal_(param, mean=0, std=0.01)  # 可以根据需要选择初始化方法和参数
                        elif 'bias' in name:
                            nn.init.constant_(param, 0)  
                else:
                    print(f"Ignoring {name} as it is not found in the model")
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.freeze_mode == 0 and epoch < args.lr_drop and epoch % 5 != 0:  ## eval every 5 epoch before lr_drop
            ...
            continue
        test_stats = evaluate_hoi(args.dataset_file, model, postprocessors, data_loader_val, args.subject_category_id, device, args)
        train_map = evaluate_hoi(args.dataset_file, model, postprocessors, data_loader_val2, args.subject_category_id, device, args)
        test_loss = train_one_epoch(
            model, criterion, data_loader_train2, optimizer, device, epoch,
            args.clip_max_norm,mode="test")
        coco_evaluator = None
        ...
        log_stats = {**{f'train_{k}': v for k, v in train_stats.items()},
                     ...
                     **{f'train_{k}': v for k, v in train_map.items()},
                     **{f'test_{k}': v for k, v in test_loss.items()},}
        log_stats2={**{f'train_{k}': train_stats[k] for k in ["lr","loss"]},
                    **{f'test_{k}': test_loss[k] for k in ["lr","loss"]},
                    **{f'test_{k}': test_stats[k] for k in ["mAP","mean max recall"]},
                    **{f'train_{k}': train_map[k] for k in ["mAP","mean max recall"]},
                    'epoch': epoch,
        ...
        if args.output_dir and utils.is_main_process():
            ...
            with (output_dir / "log2.txt").open("a") as f:
...
if __name__ == '__main__':
    ...
    # args.pretrained="work_dir/1205_cdn_l_lr_5e-5/checkpoint_last.pth"
    args.pretrained="checkpoints/origin_net/hico_cdn_l.pth"
    # args.pretrained="/home/wzy/CDN_mod/checkpoints/origin_net/checkpoint_last_random_split.pth"
