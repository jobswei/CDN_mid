...
class CDNHOI(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_obj_classes, num_verb_classes, num_queries, aux_loss=False, args=None):
        ...
        self.transformer = transformer # cdn
        hidden_dim = transformer.d_model
        self.query_embed = nn.Embedding(num_queries, hidden_dim)
        self.obj_class_embed = nn.Linear(hidden_dim, num_obj_classes + 1)
        self.no_interact = False
        self.verb_class_embed = nn.Linear(hidden_dim, num_verb_classes)
        if args.dec_layers_interaction == 0:
        ...
        if self.no_interact == False:
            self.dec_layers_interaction = args.dec_layers_interaction
        else:
            self.dec_layers_interaction = self.dec_layers_hopd
    ...
    def forward(self, samples: NestedTensor):
        ...
        outputs_sub_coord = self.sub_bbox_embed(hopd_out).sigmoid() # bbox
        outputs_obj_coord = self.obj_bbox_embed(hopd_out).sigmoid() # bbox
        outputs_obj_class = self.obj_class_embed(hopd_out) # cls -> 3
        if self.use_matching:
            outputs_matching = self.matching_embed(hopd_out) # hoi -> 1
        # import ipdb;ipdb.set_trace()
        if self.no_interact:
            assert interaction_decoder_out == None
