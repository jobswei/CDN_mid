...
class HICOEvaluator():
    def __init__(self, preds, gts, rare_triplets, non_rare_triplets, correct_mat, args):
        ...
        # image=cv2.imread(osp.join("/home/wzy/CDN/data/hico_20160224_det/images/test2015",self.gts[0]["filename"]))
        # bbox=tuple(self.gts[0]["annotations"][1]["bbox"])
        ...
        preds=self.preds
        # for i in self.preds:
        #     preds.append(filter_res_num(i,10))
        gts=str(self.gts).replace(".0"," ")
        res={"preds":preds, "gts":eval(gts)}
