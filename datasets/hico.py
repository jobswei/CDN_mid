...
class HICODetection(torch.utils.data.Dataset):
    ...
    def __init__(self, img_set, img_folder, anno_file, transforms, num_queries):
        ...
        self._valid_obj_ids = (1, 2, 3)
        self._valid_verb_ids = list(range(1, 2))
...
def build(image_set, args):
    ...
    PATHS = {
        ...
        'val2': (root / 'images' / 'train2015', root / 'annotations' / 'trainval_hico.json'),
        'train2': (root / 'images' / 'test2015', root / 'annotations' / 'test_hico.json')
    }
    CORRECT_MAT_PATH = root / 'annotations' / 'corre_hico.npy'

    img_folder, anno_file = PATHS[image_set]
    if image_set[-1].isdigit():
